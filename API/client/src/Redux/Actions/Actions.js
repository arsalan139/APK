...
export const DELETE_USER = (id, callback, errorCb) => {
  return async (dispatch) => {
...
export const DELETE_WORKER = (id, callback, errorCb) => {
  return async (dispatch) => {
    await API.delete("/admin/worker/" + id)
      .then((res) => {
        dispatch(GET_WORKERS_LIST(callback));
        SUCCESS(`Worker is Deleted!`);
      })
      .catch((error) => {
        if (error.response) {
          ERROR(error.response.data);
...
export const DELETE_WORKER_REQUEST = (id, callback, errorCb) => {
  return async (dispatch) => {
    await API.delete("/admin/workerRequest/" + id)
      .then((res) => {
